...
Build an Astro-first component library and design system that works as server-rendered HTML by default and adds client-side behavior only where interaction requires it.
...
Components include small primitives and application-ready structures such as PageHeader, Table, SearchInput, Pagination, and EmptyState. Higher-level application systems remain a later layer and do not control the core inventory.

Future Frasto Icons, Blocks, Pages, and SaaS products build on this foundation after real usage demonstrates stable needs. They are not hidden requirements of the UI package.
