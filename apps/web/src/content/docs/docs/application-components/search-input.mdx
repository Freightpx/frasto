...
- Preserve useful server submission without client JavaScript.
- Keep the enhancement local to clear-button visibility, focus restoration, and event dispatch.
- Compose existing Input, IconButton, Icon, and Spinner primitives.
- Do not add Escape-to-clear, debounce, result fetching, or URL synchronization.
- Preserve native input attributes, consumer classes, disabled state, and invalid state.
